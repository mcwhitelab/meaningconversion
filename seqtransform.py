...
class ProteinMutationEnv(gym.Env):
    ...
    def __init__(self, initial_ids, target_ids, possible_mutations, mutstate, starting_seqsim, model, tokenizer, device):
        ...
        self.best_seqsim = self.calculate_reward(self.initial_ids, self.target_ids, self.model, self.tokenizer, self.device)
    ...
    def step(self, action):
        ...
        seqsim = self.calculate_reward(self.current_ids, self.target_ids, self.model, self.tokenizer, self.device) 
        reward = seqsim - self.best_seqsim
        ...
        return self.current_ids, reward, self.mutstate, self.best_seqsim, done, {}
    ...
    def calculate_reward(self, current_ids, target_ids, model, tokenizer, device):
        ...
        with torch.no_grad():
            ...
            target_mean_embedding = get_representation(model, target_ids.unsqueeze(0), "t5", layers = [-1]).to(device)
            orig_mean_embedding = get_representation(model, current_ids.unsqueeze(0), "t5", layers = [-1]).to(device)
...
if __name__ == "__main__":
    ...
    for episode in range(num_episodes):
        ...
        while not done:
          ...
          next_ids, reward, mutstate, best_seqsim, done, _ = env.step(action)
          next_state = mutstate
    ...
    total_rewards['rank'] = total_rewards.groupby('episode')['cumulative_reward'].rank(method='dense', ascending=False)
    ...
    def apply_actions_in_order(actions, possible_mutations, target_mean_embedding, model, tokenizer, device):
        ...
        for action in actions:
            ...
            with torch.no_grad():
                ...
                current_mean_embedding = get_representation(model, current_ids.unsqueeze(0), "t5", layers = [-1]).to(device)
